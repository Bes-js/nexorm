...
async function updateMany(model, where, update, rules, options, schema) {
    ...
    try {
        var findedValues = await model.findAll({ where: where, transaction: options.$transaction?.trx });
        ...
        for (var findedValue of findedValues) {
            if (!findedValue && options.$upsert) {
                ...
                updatedValues.push(await (0, build_1.build)(model, { ...where, ...buildValue }, { $transaction: options.$transaction, $hooks: options?.$hooks ?? true }));
                continue;
            }
            ;
            if (!findedValue)
            ...
            ;
            updatedValues.push((await findedValue.update(value, { transaction: options.$transaction?.trx, hooks: options?.$hooks ?? true })).dataValues);
        }
        ;

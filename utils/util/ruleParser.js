...
async function ruleParser(value, rules, targetValue) {
    ...
    if (rules[value].$noWhitespace) {
        if (targetValue && targetValue?.match(/\s/))
    ...
    if (rules[value].$alpha) {
        if (targetValue && !targetValue?.match(/^[a-zA-Z]*$/))
